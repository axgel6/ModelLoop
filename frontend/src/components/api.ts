...
export async function apiGetMyFeatures(): Promise<Record<string, boolean>> {
  const res = await withRefresh(() =>
    fetch(`${API_URL}/api/v1/auth/features`, { headers: authHeaders() }),
  );
  if (!res.ok) return {};
  return res.json();
}

export async function apiGetGuestFeatures(): Promise<Record<string, boolean>> {
  const res = await fetch(`${API_URL}/api/v1/auth/features/guest`);
  if (!res.ok) return {};
  return res.json();
}
...
export async function apiAdminToggleAccess(
  userId: string,
...
export async function apiAdminSetRole(
  userId: string,
  role: string,
): Promise<AdminUser> {
  const res = await withRefresh(() =>
...
export async function apiAdminGetAuditLogs(
  ...
  const res = await withRefresh(() =>
    fetch(`${API_URL}/api/v1/admin/audit-logs?${params}`, {
      headers: authHeaders(),
    }),
  );
...
export async function apiAdminGetFeatureFlags(): Promise<FeatureFlag[]> {
  const res = await withRefresh(() =>
    fetch(`${API_URL}/api/v1/admin/feature-flags`, { headers: authHeaders() }),
  );
  if (!res.ok) throw new Error("Failed to fetch feature flags");
  return res.json();
}

export async function apiAdminUpdateFeatureFlag(
  name: string,
  changes: Partial<
    Pick<
      FeatureFlag,
      "guest_enabled" | "free_enabled" | "pro_enabled" | "admin_enabled"
    >
  >,
): Promise<FeatureFlag> {
  const res = await withRefresh(() =>
    fetch(`${API_URL}/api/v1/admin/feature-flags/${encodeURIComponent(name)}`, {
      method: "PATCH",
      headers: { ...authHeaders(), "Content-Type": "application/json" },
